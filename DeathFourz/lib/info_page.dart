...
class _InfoPageState extends State<InfoPage> {
  ...
  Future<void> _fetchServerInfo() async {
    try {
      final res = await http.get(
        Uri.parse('http://152.42.219.134:2106/getServerInfo?key=${widget.sessionKey}'),
      );
  ...
  Future<void> _checkApiPing() async {
    ...
    try {
      final res = await http.get(
        Uri.parse('http://152.42.219.134:2106/ping?key=${widget.sessionKey}'),
  ...
  Widget build(BuildContext context) {
    ...
    if (isLoading) {
      return Scaffold(
        ...
        body: const Center(
          child: CircularProgressIndicator(color: Color(0xFFEA80FC)),
        ),
      );
    }
            ...
            Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.gavel, color: Color(0xFFEA80FC), size: 20),
                ),
