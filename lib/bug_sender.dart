...
class _BugSenderPageState extends State<BugSenderPage> {
  ...
  Future<void> _fetchSenders() async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://152.42.219.134:2106/mySender?key=${widget.sessionKey}"),
  ...
  Future<void> _addSender(String number) async {
    ...
    try {
      final response = await http.get(
        Uri.parse("http://152.42.219.134:2106/getPairing?key=${widget.sessionKey}&number=$number"),
      );
    ...
    if (confirmed == true) {
      ...
      try {
        final response = await http.delete(
          Uri.parse("http://152.42.219.134:2106/deleteSender?key=${widget.sessionKey}&id=$senderId"),
        );
